...
- **D-10:** Today every adapter refusal collapses to `bad_request`, which tells a player nothing. `ErrorDetailSchema` is widened with a **closed enum** of rule-refusal reasons and `mapAdapterError` maps the adapter's typed errors onto it 1:1. This stays inside D-08-from-Phase-2's rule that error frames carry no state: a closed vocabulary, never free text, never game data.
  **Verified 2026-09-16 by reading `packages/rules/src/adapter.ts`** — `AdapterError` has exactly these 8 members, and the mapping must cover all of them: `not_your_turn`, `invalid_action`, `game_over`, `card_not_in_hand`, `no_clue_tokens`, `clue_touches_nothing`, `clue_target_invalid`, `discard_at_max_clues`. Note `mapAdapterError` is currently **parameterless** and returns a constant (`room-state.ts`), so its signature changes; check every call site with a build, not a grep alone.
...
- **D-15:** RT-09 is proven by **deliberately double-sending the same `actionId`** at the **socket level**, extending the existing `wrangler dev` + raw `ws` harness in `apps/worker/src/room-do.test.ts` (the same one that proves the D-17 eviction path). Playwright offers no reliable hook for forcing a byte-identical duplicate frame, so it is the wrong tool for this one.
  **Assertion targets — verified 2026-09-16 by reading the code, not by report.** `HanabiView` declares `history: HistoryEntryView[]` (`packages/rules/src/hanabi/state.ts`) and `toHanabiPlayerView` populates it (`projection.ts` maps `state.history` and returns it in both view branches). So history **is** on the wire and **is** a valid assertion target: after a duplicated send, `history.length` must be unchanged. Assert it alongside `clueTokens`, `activeSeatId`/`isYourTurn`, `deckCount` and the target hand's clue facts.
  *Correction trail, kept deliberately:* this decision first said "history length", then was edited to say history is server-only and unobservable (following `04-RESEARCH.md`'s claim), and is now restored. The research was wrong on this point and the orchestrator propagated it without re-reading the file. Phase 3's D-19 makes history **public-facts-only** — a draw records just a card id — which is what allows it on the wire; it does not keep history off the wire. Do **not** add a debug hook to expose internal state for a test.
