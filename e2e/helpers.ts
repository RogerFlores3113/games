 ...
 * `own-hand-slot-` prefix. The overlay carries no `data-hints` of its own
 * (that attribute lives on the slot button itself), so a bare-prefix
 * `[data-hints="true"]` locator would silently count it as a second match;
 * the overlay is also conditionally rendered (it disappears when the card
 * has no positive clues, or when "keep hints visible" is off and the next
 * player has acted), so a bare-prefix slot COUNT drifts with hint
 * visibility rather than with the hand. Same shape as `OTHER_HAND_SELECTOR`
